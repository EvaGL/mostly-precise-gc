...
template <class T> T* gc_new (size_t count=1)  
{
        ...
        if (counter >= 50000000) /* if occupated place more than 500000000 lets start to collect*/
        {
                mark_and_sweep();
                counter = 0;
        }
        ...
        if (count == 1) {  /* just pointer on one-word type*/
                ...
                *((size_t*)(res + sizeof(meta<T>))) =  reinterpret_cast <size_t> (new (res) meta<T>);  /* initialize meta in obj */
                ...
                m_inf->mbit = 0;
                m_inf->ptr = (T*)(res + sizeof(base_meta*) + sizeof(meta<T>)); /*set pointer on value of object(gc_ptr)*/
        }
        else {  /* if gc_new <name_of_type> (2..n) - size of sruct more than 1 word, the same actions except allocating space*/
                res = malloc(sizeof(T) * count + sizeof(void *) + sizeof(meta<T>));
                new (res + sizeof(void*) + sizeof(meta<T>)) T[count]; /* here we calling constructor from T[count]*/
        ...
        ptr_in_heap.push_back(res + sizeof(base_meta*) + sizeof(meta<T>));  /* adding ptr in list of heap pointers*/
        new_active = false;  
